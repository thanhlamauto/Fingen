...
\begin{frame}{Team Contribution}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.32\textwidth}
      \begin{block}{Thanh Lam Nguyen\\{\small 20235519}}
        \begin{itemize}
          \item Problem framing and related-work positioning.
          \item Stage-1 identity prior setup and ridge extraction pipeline.
          \item SD302A experiment organization and ablation integration.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{block}{Chi Bang Pham\\{\small 20235477}}
        \begin{itemize}
          \item Stage-2 ridge-conditioned renderer setup.
          \item DB1--DB4 LoRA transfer and pose-mask augmentation.
          \item Qualitative figures and live demo preparation.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{block}{Phu An Nguyen\\{\small 20235466}}
        \begin{itemize}
          \item Recognition evaluation protocol and metric tables.
          \item LightGlue and t-SNE diagnostic analysis.
          \item Report, slide, appendix, and reference consolidation.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  \smallnote{All members jointly reviewed the method design, experiment results,
  final report, and presentation. Contributions are treated as equal.}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{Core story}
        \begin{itemize}
          \item 0.5 min: team contribution and roles
          \item 1.5 min: problem and target-sensor setting
          \item 2 min: related work and competitor methods
          \item 3.5 min: two-stage ridge-conditioned method
          \item 0.5 min: dataset statistics and samples
          \item 4 min: SD302A and FVC experiments
...
\begin{frame}{Related Work: Where We Fit}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      \begin{block}{Synthetic fingerprint generators}
        \begin{itemize}
          \item \textbf{SFinGe}: hand-designed ridge flow, shape, and noise.
          \item \textbf{PrintsGAN}: GAN-based synthetic identities with multiple
          impressions per finger.
          \item \textbf{FPGAN-Control}: controllable GAN; disentangles identity
          from appearance attributes such as sensor and pressure.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{block}{Diffusion and control}
        \begin{itemize}
          \item \textbf{Latent diffusion}: denoise in compressed VQ-VAE latent
          space for high-resolution synthesis.
          \item \textbf{ControlNet}: inject spatial conditions such as ridge maps.
          \item \textbf{LoRA}: adapt a frozen generator with low-rank weights.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  \smallnote{Our design combines these ideas, but keeps the target-sensor
  adaptation small and fingerprint-specific.}
\end{frame}

\begin{frame}{Competitor Methods and Gaps}
  \centering
  {\scriptsize
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.18\linewidth}
      ...
      \toprule
      Method & How it generates fingerprints & Gap for our target setting \\
      \midrule
      PrintsGAN
      & GAN learns synthetic fingers and repeated impressions.
      & Limited explicit sensor-transfer control. \\
      FPGAN-Control
      & Controllable GAN separates identity and appearance factors.
      & Main SD302A baseline, but new-sensor adaptation is not a tiny-adapter path. \\
      GenPrint
      & Stable-Diffusion-style model with text and image conditions.
      & Powerful, but large backbone and broad training recipe. \\
      Ridge Scale Aligned Diffusion
      & Diffusion identity/style synthesis with ridge-scale diagnostics.
      & Useful reference, but not our lightweight LoRA transfer path. \\
      \bottomrule
  \end{tabular}}
  \vspace{0.6em}
  \smallnote{\textbf{Our response:} keep identity generation, ridge control,
  and sensor rendering separate; freeze the structural path; train only a small
  Stage-2 sensor LoRA.}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Quick retraining sweep completed for DB1 and DB2 under the
        available time/resource budget.
        \item Use EER on DB\emph{x}\_B to choose the smallest rank close to the
        best rank for that sensor.
      \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
      \resizebox{\linewidth}{!}{%
        \begin{tabular}{lcc}
          ...
          \bottomrule
        \end{tabular}}
      \smallnote{EER is where false accepts and false rejects are equal; lower is better.}
      \vspace{0.65em}
      \[
        \begin{gathered}
...
\begin{frame}{Datasets: NIST SD302A and FVC 2004}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{block}{NIST SD302A}
        \scriptsize
        \begin{tabular}{@{}>{\bfseries}p{0.30\linewidth}p{0.62\linewidth}@{}}
          Role & Main benchmark \\
          Sensors & Challenger sensors A--H \\
          Split & 1,600 train / 400 held-out fingers \\
          Real images & 10,896 train / 2,734 held-out \\
          Synthetic & 2,000 IDs $\times$ 10 impressions \\
        \end{tabular}
      \end{block}
      \includegraphics[width=\linewidth,height=0.30\textheight,keepaspectratio]{figures/qc_sensor_overview.png}
      \smallnote{SD302A stresses large variation in frame geometry, background,
      ridge contrast, binarization artifacts, and absolute placement.}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{block}{FVC 2004}
        \scriptsize
        \begin{tabular}{@{}>{\bfseries}p{0.30\linewidth}p{0.62\linewidth}@{}}
          Role & Cross-sensor transfer test \\
          Databases & DB1--DB4 sensor styles \\
          Identities & 100 per database \\
          LoRA fit & 80 DB\emph{x}\_A images per DB \\
          Evaluation & DB\emph{x}\_B verification only \\
        \end{tabular}
      \end{block}
      \includegraphics[width=\linewidth,height=0.30\textheight,keepaspectratio,trim={0 150bp 0 1100bp},clip]{figures/fvc_lora_db1_real_synth_page.png}
      \smallnote{Example real DB1\_A impressions are shown; DB1--DB4 each get a
      separate Stage-2 LoRA adapter and are evaluated on DB\emph{x}\_B.}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{block}{SD302A benchmark}
        \begin{itemize}
          \item Compare real, synthetic-only, and real-plus-synthetic training.
          \item Keep the held-out identities fixed across recognizer backbones.
          \item Synthetic identities come from Stage 1; target sensor style
          comes from Stage 2 rendering.
    ...
    \begin{column}{0.49\textwidth}
      \begin{block}{FVC 2004 transfer}
        \begin{itemize}
          \item Train one DB-specific Stage-2 LoRA adapter per database.
          \item Use DB\emph{x}\_A for LoRA fitting and classifier training.
          \item Evaluate recognition on DB\emph{x}\_B real images only.
...
\begin{frame}{Result 1: SD302A Recognition Utility}
  ...
  \resizebox{0.88\linewidth}{!}{%
    \begin{tabular}{lccc}
      ...
      \bottomrule
    \end{tabular}}
  \smallnote{TAR@FAR=0.1\% is the true-accept rate at a threshold allowing
  only 0.1\% false accepts; higher is better.}
  \vspace{0.1em}
  \begin{itemize}
...
\begin{frame}{Result 3: FVC 2004 Cross-Sensor Recognition}
  ...
  \resizebox{0.78\linewidth}{!}{%
    \begin{tabular}{lcccc}
      ...
      \bottomrule
    \end{tabular}}
  \smallnote{TAR@FAR=0.1\% measures accepted genuine pairs under the same
  0.1\% false-accept constraint; higher is better.}
  \vspace{0.1em}
  \begin{itemize}
    ...
    \begin{column}{0.47\textwidth}
      \begin{block}{Key ablation numbers}
        \begin{itemize}
          ...
          \item Quick DB1/DB2 rank sweep: rank 4 gives the lowest EER among
          $k\in\{1,2,4,8\}$ in the reduced setting.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Limitations}
  \begin{itemize}
    ...
    \item Main recognition runs fix rank-8 LoRA for all FVC sensors due to
    time/resource constraints; the rank sweep is preliminary and DB1/DB2-only.
