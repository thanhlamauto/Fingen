  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      ...
      \begin{block}{Trainable per new sensor}
        Reported runs use rank-8 LoRA only on Stage-2 UNet middle self-attention
        $qkv$ and output projection.
      \end{block}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{itemize}
        \item Reported rank-8 LoRA: \key{12.3K parameters}.
...
\begin{frame}{LoRA Rank: Sensor-Specific Capacity}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \end{block}
      \begin{itemize}
        \item Rank controls how many sensor-style directions can be expressed.
        \item Different sensors can need different retained update energy.
        \item Use rank-8 checkpoint as a high-rank pilot, then inspect singular
        value energy.
      \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
      ...
      \[
        \begin{aligned}
        k_s^\star=\min\{k:\;&P(k)\le B_s,\\
        &\mathcal{J}_s(k)\le \mathcal{J}_{s,\min}+\epsilon\}.
        \end{aligned}
      \]
      \smallnote{$\mathcal{J}_s$ combines validation diffusion loss, style
      distance, LightGlue structure score, and parameter cost.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Efficiency and Practicality}
  ...
  \resizebox{0.80\linewidth}{!}{%
    \begin{tabular}{lrlr}
      ...
      \midrule
      Trainable rank-8 LoRA / sensor & \textbf{12.3K} & Logged generation & 1.05 s/image \\
      \bottomrule
    \end{tabular}}
  \vspace{0.9em}
  \begin{itemize}
...
\begin{frame}{Limitations}
  \begin{itemize}
    ...
    \item Reported recognition runs use a fixed rank-8 LoRA for all FVC sensors;
    a full retraining sweep over rank remains future work.
